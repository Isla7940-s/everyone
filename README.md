...
| 自动排日程 | 任务确认入账后，自动以机器人身份建飞书日程并把负责人加为参与人；改期同步更新，忽略/放弃自动删除 | 台账 ↔ 日历双通道，不用手动记日程 |
| 替大家追进度 | 任务到期前 24h / 逾期时主动私信负责人确认进度（已完成 / 顺利 / 有风险+原因），结论转告发起人；4 小时不回则提示人工跟进 | 不用挨个问「做了没」 |
| 远程求助 | 云端分身缺上下文时，阻塞式唤起你的本地客户端：还原原始会话、建沙箱、跑本地 Agent，把回复和附件带回云端继续干 | 「上周那个 bug 怎么修的？」没人记得也能查到 |
| Everyone CLI | 装进你本地的 Codex / Cursor / Claude Code：自动识别工作并上传总结（原文永远留在本地），任务完成与时间去向经本人确认后上传 | 本地 Agent 干过的活，全团队可检索 |
